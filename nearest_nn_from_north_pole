...
def find_gift_to_deliver(current_load_gifts, current_location):
    ...
    for gift in current_load_gifts:
        ...
        weight_for_weariness += gift_weight
    ...
    gift_to_deliver_weariness = gift_to_deliver_distance * (weight_for_weariness + SLEIGH_WEIGHT)
    return gift_to_deliver, gift_to_deliver_weariness

total_weariness = 0

df = pd.read_csv(r"gifts.csv")
df['DistanceFromPole'] = df.apply(lambda row: haversine(NORTH_POLE, (row['Latitude'],row['Longitude'])) , axis=1)
df = df.sort_values(by='DistanceFromPole', ascending=[False])
...
submission.to_csv("submission_nn_approach_nearest_to_pole.csv", index=False)
